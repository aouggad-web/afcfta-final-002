...
OUT_PATH = os.path.join(
    os.path.dirname(os.path.dirname(__file__)),
    'backend', 'data', 'crawled', 'DZA_tariffs.json'
)
...
def build_sub_position(row: dict) -> dict | None:
    ...
    daps_rate = daps_corr if daps_corr is not None else (daps_base if daps_base is not None else 0.0)
    ...
    formalities = [f.strip() for f in formalities_raw.split('|') if f.strip()] if formalities_raw else []
...
def main():
    ...
    print(f"Validated: {validated_count} | With DAPS: {has_daps} | With advantages: {has_advantages}")
