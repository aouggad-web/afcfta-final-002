...
def test_faostat_unido_endpoints():
    ...
    try:
        ...
        if response.status_code == 200:
            stats = response.json()
            if (
                stats.get("total_countries") == 54
                and stats.get("total_commodities") == 47
                and stats.get("data_year") == 2023
            ):
                log_result(
                    "FAOSTAT Statistics",
                    True,
                    f"Statistiques validées - {stats['total_countries']} pays, {stats['total_commodities']} commodités, année {stats['data_year']}",
                )
            else:
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            if (
                data.get("country_name") == "Côte d'Ivoire"
                and data.get("region") == "Afrique de l'Ouest"
                and "Cacao" in str(data.get("main_crops", ""))
            ):
                log_result(
                    "FAOSTAT CIV",
                    True,
                    f"Données CIV validées - {data['country_name']}, Cacao présent",
                )
            else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if (
                data.get("country_name") == "Égypte"
                and "Blé" in production_str
                and "Riz" in production_str
            ):
                log_result(
                    "FAOSTAT EGY",
                    True,
    ...
    try:
        response = session.get(
            f"{BASE_URL}/production/faostat/top-producers/Cacao", timeout=TIMEOUT
        )
        if response.status_code == 200:
            data = response.json()
            ...
            if (
                len(producers) >= 2
                and producers[0].get("country") == "CIV"
                and producers[1].get("country") == "GHA"
            ):
                log_result(
                    "FAOSTAT Top Producers Cacao",
                    True,
                    f"Classement validé - {len(producers)} producteurs, CIV #1, GHA #2",
                )
            else:
                log_result(
                    "FAOSTAT Top Producers Cacao",
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if len(commodities) > 0:
                log_result(
                    "FAOSTAT Commodities", True, f"Liste validée - {len(commodities)} commodités"
                )
            else:
        ...
        if response.status_code == 200:
            ...
            if stats.get("total_countries") == 54 and abs(mva - 289.9) <= 10:
                log_result(
                    "UNIDO Statistics",
                    True,
                    f"Statistiques validées - {stats['total_countries']} pays, MVA ${mva}B",
                )
            else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if (
                data.get("country_name") == "Maroc"
                and abs(mva - 32500) <= 1000
                and abs(gdp_pct - 24.8) <= 2
            ):
                log_result(
                    "UNIDO MAR", True, f"Données MAR validées - MVA ${mva}M, MVA/PIB {gdp_pct}%"
                )
            else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if (
                data.get("country_name") == "Afrique du Sud"
                and "automobile" in top_sectors_str.lower()
            ):
        ...
        if response.status_code == 200:
            ...
            if all(country in top_countries for country in expected_top):
                log_result(
                    "UNIDO Ranking",
                    True,
                    f"Classement validé - {len(ranking)} pays, ZAF/EGY/NGA dans le top",
                )
            else:
                log_result(
                    "UNIDO Ranking", False, f"Classement incorrect", {"top_5": top_countries}
                )
        else:
        ...
        if response.status_code == 200:
            ...
            if not missing_codes:
                log_result(
                    "UNIDO ISIC Sectors",
                    True,
                    f"Classification ISIC validée - {len(sectors)} secteurs, codes 10-33 présents",
                )
            else:
