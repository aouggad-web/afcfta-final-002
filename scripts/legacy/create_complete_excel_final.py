...
def create_complete_excel():
    ...
    for _, row in df_2024.iterrows():
        ...
        print(
            f"   ✅ {pays}: PIB {econ_data.get('pib_2024', 0):.1f}Mds, S&P: {ratings.get('sp', 'NR')}, Risque: {complete_entry['Risque_Global']}"
        )
    ...
    with pd.ExcelWriter("/app/ZLECAf_COMPLET_AVEC_NOTATIONS.xlsx", engine="openpyxl") as writer:
        ...
        guide_data = [
            ...
            [
                ...
                "États-Unis",
                "AAA à D",
                "Référence mondiale - Agence américaine",
            ],
    ...
    # Top 10 PIB
    top_pib = df_complete.nlargest(10, "PIB_2024_Mds_USD")[
        ["Pays", "PIB_2024_Mds_USD", "S_P_Rating"]
    ]
    print("\nTop 10 économies africaines ZLECAf:")
    for _, row in top_pib.iterrows():
        print(
            f"   {row['Pays']:25s}: {row['PIB_2024_Mds_USD']:8.1f} Mds USD (S&P: {row['S_P_Rating']})"
        )
