...
class ZLECAf2024DataTester:
    def __init__(self):
        ...
        self.session.headers.update(
            {"Content-Type": "application/json", "User-Agent": "ZLECAf-2024-Data-Tester/1.0"}
        )
        self.results = []
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for country in countries:
                    if (
                        country["gdp_2024"] == 0
                        or country["exports_2024"] == 0
                        or country["imports_2024"] == 0
                    ):
    ...
    def test_enhanced_statistics_2024(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                missing_sections = [
                    section for section in required_2024_sections if section not in stats
                ]
                ...
                missing_evolution_fields = [
                    field for field in required_evolution_fields if field not in trade_evolution
                ]
                ...
                if (
                    trade_evolution["intra_african_trade_2024"]
                    <= trade_evolution["intra_african_trade_2023"]
                ):
                    self.log_result(
                        "Enhanced Statistics 2024",
                        False,
                ...
                exporters_by_code = {
                    exp.get("country", ""): exp for exp in top_exporters if isinstance(exp, dict)
                }

                expected_top_exporters = ["ZAF", "NGA", "AGO"]  # Codes ISO3
                found_exporters = [
                    code for code in expected_top_exporters if code in exporters_by_code
                ]
                ...
                self.log_result(
                    ...
                    f"Statistiques enrichies 2024 complètes avec évolution commerciale {trade_evolution['growth_rate_2023_2024']}%",
                    {
                        ...
                        "projections_available": (
                            ["2025", "2030"]
                            if "2025" in projections and "2030" in projections
                            else []
                        ),
                    },
                )
    ...
    def test_country_profile_enriched_zaf(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                missing_fields = [
                    field for field in required_enriched_fields if field not in profile
                ]
                ...
                required_commerce_fields = [
                    ...
                    "export_partners",
                    "import_partners",
                    "exports_2024_billion_usd",
                    "imports_2024_billion_usd",
                    "trade_balance_2024_billion_usd",
                ]

                missing_commerce_fields = [
                    field for field in required_commerce_fields if field not in projections
                ]
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                required_2024_fields = [
                    ...
                    "computation_order_ref",
                    "confidence_level",
                ]

                missing_2024_fields = [
                    field for field in required_2024_fields if field not in calculation
                ]
                ...
                if not calculation.get("computation_order_ref") or not calculation.get(
                    "confidence_level"
                ):
                    self.log_result(
                        "Updated Tariff Calculation 2024",
