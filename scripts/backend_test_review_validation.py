...
class ZLECAfReviewValidator:
    def __init__(self):
        ...
        self.session.headers.update(
            {"Content-Type": "application/json", "User-Agent": "ZLECAf-Review-Validator/1.0"}
        )
        self.results = []
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for country in countries:
                    if (
                        country.get("gdp_2024", 0) > 0
                        and country.get("exports_2024", 0) > 0
                        and country.get("hdi_2024", 0) > 0
                    ):
            ...
            self.log_result(
                "Trade Performance Endpoint", False, f"Erreur d'accès: {str(e)}", {"error": str(e)}
            )
    ...
    def validate_enhanced_statistics_endpoint(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if not (
                    "intra_african_trade_2023" in trade_evolution
                    and "intra_african_trade_2024" in trade_evolution
                ):
                    self.log_result(
                        "Enhanced Statistics Endpoint",
                        False,
                ...
                exporters_dict = {
                    exp.get("country", ""): exp.get("exports", 0) for exp in top_exporters
                }
                ...
                if "ZAF" in exporters_dict:
                    ...
                    validation_checks.append(
                        ("ZAF", zaf_exports, 108.2, abs(zaf_exports - 108.2) < 5)
                    )

                if "NGA" in exporters_dict:
                    nga_exports = exporters_dict["NGA"]
                    validation_checks.append(
                        ("NGA", nga_exports, 68.5, abs(nga_exports - 68.5) < 5)
                    )

                if "AGO" in exporters_dict:
                    ago_exports = exporters_dict["AGO"]
                    validation_checks.append(
                        ("AGO", ago_exports, 42.8, abs(ago_exports - 42.8) < 5)
                    )
