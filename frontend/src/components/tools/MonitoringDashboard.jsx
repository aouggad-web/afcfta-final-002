  ...
  return (
    <div className="space-y-6">
      <Card className="afcfta-dark-gradient border-2" style={{borderColor:'rgba(96,165,250,0.3)'}}>
        <CardHeader>
          <CardTitle className="text-2xl text-[var(--info)]">{t.title}</CardTitle>
          <CardDescription>{t.subtitle}</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div className="rounded-xl p-4 shadow text-center" style={{background:'rgba(255,255,255,0.06)'}}>
              <div className="text-3xl font-bold text-[var(--info)]">{stats?.country_count || 0}</div>
              <div className="text-sm" style={{color:'var(--afcfta-muted)'}}>{t.totalCountries}</div>
            </div>
            <div className="rounded-xl p-4 shadow text-center" style={{background:'rgba(255,255,255,0.06)'}}>
              <div className="text-3xl font-bold text-[var(--success)]">{formatNumber(stats?.total_tariff_lines)}</div>
              <div className="text-sm" style={{color:'var(--afcfta-muted)'}}>{t.totalLines}</div>
            </div>
            <div className="rounded-xl p-4 shadow text-center" style={{background:'rgba(255,255,255,0.06)'}}>
              <div className="text-3xl font-bold text-[var(--violet)]">{formatNumber(stats?.total_sub_positions)}</div>
              <div className="text-sm" style={{color:'var(--afcfta-muted)'}}>{t.totalSub}</div>
            </div>
            <div className="rounded-xl p-4 shadow text-center" style={{background:'rgba(255,255,255,0.06)'}}>
              <div className="text-3xl font-bold text-[var(--terra)]">{formatNumber(stats?.total_positions)}</div>
          ...
          {subscriptionNotice && (
            ...
            >
              ...
              <div>
                <div className="font-semibold text-[var(--gold)]">{t.subscriptionTitle}</div>
                <p className="text-[var(--gold)] text-sm">{subscriptionNotice}</p>
              </div>
            </div>
          )}
      ...
      {subscriptionNotice ? null : loading ? (
        <div className="text-center py-8 text-[var(--afcfta-muted)]">Loading...</div>
      ) : !stats?.countries?.length ? (
        <div className="text-center py-8 text-[var(--afcfta-muted)]">{t.noData}</div>
      ) : (
        <Card>
          <CardContent className="p-0">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-[var(--afcfta-card2)] border-b">
                  <tr>
                    {[
                      ...
                      <th
                        ...
                        className="px-3 py-2 text-left cursor-pointer hover:bg-[var(--afcfta-card2)] select-none"
                      >
                ...
                <tbody>
                  {sortedCountries.map((c, i) => (
                    <tr key={c.code} className={i % 2 === 0 ? 'bg-[var(--afcfta-card)]' : 'bg-[var(--afcfta-card2)]'}>
                      <td className="px-3 py-2 font-bold text-[var(--info)]">{c.code}</td>
                      <td className="px-3 py-2">{formatNumber(c.tariff_lines)}</td>
                      <td className="px-3 py-2 text-[var(--violet)]">{formatNumber(c.sub_positions)}</td>
                      <td className="px-3 py-2 font-semibold text-[var(--terra)]">{formatNumber(c.total_positions)}</td>
                      <td className="px-3 py-2">
                        <div className="flex items-center gap-2">
                          <div className="w-16 bg-[var(--afcfta-card2)] rounded-full h-2">
                            <div
                              className="bg-green-500 h-2 rounded-full"
                              style={{ width: `${Math.min(100, (c.lines_with_sub_positions / c.tariff_lines) * 100)}%` }}
                            ></div>
                          </div>
                          <span className="text-xs text-[var(--afcfta-muted)]">{Math.round((c.lines_with_sub_positions / c.tariff_lines) * 100)}%</span>
                        </div>
                      </td>
                      <td className="px-3 py-2">{c.vat_rate}%</td>
                      <td className="px-3 py-2">{c.dd_avg}%</td>
                      <td className="px-3 py-2">{c.chapters}</td>
                      <td className="px-3 py-2 text-xs text-[var(--afcfta-muted)]">{formatDate(c.generated_at)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
