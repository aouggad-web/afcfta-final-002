...
### August 14, 2026 (session 7) — Fix bug (3e occurrence, cause réelle) : cookie CSRF SameSite=Strict incompatible avec l'iframe de preview
- Le fix session 6 (URL backend) n'a pas suffi : le vrai coupable était le cookie CSRF (`csrf_token`) posé avec `SameSite=Strict`. Quand l'app est visualisée dans l'iframe d'aperçu Emergent (document top-level = un autre site, ex. app.emergent.sh), les navigateurs bloquent l'envoi d'un cookie `Strict` même pour les requêtes same-origin de l'iframe vers son propre domaine — le double-submit CSRF ne pouvait donc jamais aboutir dans ce contexte, provoquant l'échec "Vérifiez votre accès réseau" à l'inscription/connexion.
- Fix : `SameSite=Strict` → `SameSite=None` (+ `Secure`, obligatoire avec `None`) pour le cookie CSRF, côté backend (`backend/middlewares/csrf_protection.py`) ET frontend (`frontend/src/services/csrf.js`, fonction `persistReadableToken`, avec repli en `Lax` si servi en HTTP non chiffré). Sûr car la protection double-submit repose sur le fait que seul le JS same-origin peut lire le cookie et l'écho dans le header — pas sur le blocage SameSite.
- Vérifié par `auto_frontend_testing_agent` : inscription réussie sur l'URL directe (non-régression), cookie confirmé `SameSite=none; Secure` par curl, aucune erreur console CORS/CSRF.
- Diagnostic clé qui a permis de trouver la cause : `curl -v` sur `/api/auth/register` sans passer par `/api/health` retourne normalement `403 CSRF token missing` (comportement attendu, la protection fonctionne) — la vraie fuite se voyait en comparant les attributs `Set-Cookie` du cookie de session (`access_token`, réécrit par l'infra en `SameSite=None; Secure; Partitioned` pour supporter l'iframe) contre ceux du cookie CSRF applicatif (resté `SameSite=strict`, non couvert par cette réécriture infra).
