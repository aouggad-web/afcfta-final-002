...
class TestStatisticsEndpoint:
    ...
    def test_statistics_trade_evolution_data(self):
        ...
        assert (
            trade_evo.get("intra_african_trade_2024") == 218.7
        ), f"Expected 218.7, got {trade_evo.get('intra_african_trade_2024')}"
        assert (
            trade_evo.get("total_exports_2024") == 553.7
        ), f"Expected 553.7, got {trade_evo.get('total_exports_2024')}"
        print(
            f"✓ Trade evolution data verified: Intra-African=${trade_evo['intra_african_trade_2024']}B, Total Exports=${trade_evo['total_exports_2024']}B"
        )
    ...
    def test_statistics_top_exporters(self):
        ...
        print(
            f"✓ Top exporters verified: {len(exporters)} countries, top is {first_exporter['name']}"
        )
...
class TestUNCTADLSCIEndpoint:
    ...
    def test_lsci_top_countries(self):
        ...
        assert (
            morocco["rank_africa"] == 1
        ), f"Morocco should be rank 1 in Africa, got {morocco['rank_africa']}"
...
class TestCountryProfileEndpoint:
    ...
    def test_country_profile_dz_data(self):
        ...
        print(
            f"✓ Country profile DZ verified: {data['country_name']}, GDP=${gdp/1e9:.1f}B, Pop={pop/1e6:.1f}M"
        )
