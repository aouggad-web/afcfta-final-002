...
MAJOR_ECONOMIES = {
    "NG": {
        ...
        "expected_rank": 1,
        "min_gdp_2024": 250,
    },  # CSV has 253B, country_data.py has 334B - DATA DISCREPANCY
    "ZA": {
        "name": "South Africa",
        "iso3": "ZAF",
        "expected_rank": 2,
...
class TestUNCTADPortStatistics:
    ...
    def test_unctad_ports_endpoint(self):
        ...
        print(
            f"   Total African Port Throughput: {data.get('total_african_port_throughput_teu_2024', 'N/A'):,} TEU"
        )
    ...
    def test_tanger_med_throughput(self):
        ...
        assert (
            min_teu <= throughput <= max_teu
        ), f"Tanger Med TEU should be ~{expected_teu:,}, got {throughput:,}"
    ...
    def test_top_african_ports_ranking(self):
        ...
        for i, port in enumerate(top_ports[:5]):
            print(
                f"   {i+1}. {port.get('port')} ({port.get('country')}): {port.get('throughput_teu', 0):,} TEU"
            )
        ...
        for i in range(len(top_ports) - 1):
            ...
            assert (
                current_teu >= next_teu
            ), f"Ports not sorted correctly: {top_ports[i]['port']} < {top_ports[i+1]['port']}"
...
class TestUNCTADLSCI:
    ...
    def test_lsci_endpoint(self):
        ...
        for country in data[:5]:
            print(
                f"   {country.get('rank_africa')}. {country.get('country_fr')} - LSCI: {country.get('lsci_2024')} (Global #{country.get('rank_global')})"
            )
    ...
    def test_morocco_lsci_rank_1(self):
        ...
        assert (
            morocco.get("rank_africa") == 1
        ), f"Morocco should be #1 in Africa LSCI, got #{morocco.get('rank_africa')}"
    ...
    def test_egypt_lsci_rank_2(self):
        ...
        assert (
            egypt.get("rank_africa") == 2
        ), f"Egypt should be #2 in Africa LSCI, got #{egypt.get('rank_africa')}"
