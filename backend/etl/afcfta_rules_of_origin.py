...
YTB_HEADINGS = [
    ...
    "5407",
    "5408",  # Woven fabrics of man-made filaments
    "5512",
    "5513",
    "5514",
    "5515",
    "5516",  # Woven fabrics of man-made staple fibres
    "5801",
    "5802",
    "5803",
    "5804",
    "5806",
    "5810",  # Special woven fabrics
    "6001",
    "6002",
    "6003",
    "6004",
    "6005",
    "6006",  # Knitted fabrics
    "6301",
    "6302",
    "6303",
    "6304",
    "6305",
    "6306",  # Made-up textile articles
    "8701",
    "8703",
    "8704",
    "8705",
...
try:
    ...
    _roo_json_path = _os.path.join(
        _os.path.dirname(__file__), "..", "data", "zlecaf_rules_of_origin.json"
    )
    ...
    if _roo_json.get("headings"):
        HEADING_RULES = {
            k: _to_legacy_chapter(v)
            for k, v in _roo_json["headings"].items()
            if v.get("status") != "YTB"
        }
...
def get_chapter_rule(chapter: str, lang: str = "fr") -> dict:
    ...
    if chapter in CHAPTER_RULES:
        ...
        return {
            ...
            "alt_name": (
                ORIGIN_TYPES.get(rule.get("alt"), {}).get(lang, "") if rule.get("alt") else None
            ),
...
def get_rule_of_origin(hs_code: str, lang: str = "fr") -> dict:
            ...
            "alternative_rule": (
                {
                    ...
                    "description": (
                        ORIGIN_TYPES.get(alt_code, {}).get(lang, alt_code) if alt_code else None
                    ),
                }
                if alt_code
                else None
    ...
    if is_ytb(hs_code):
        return {
            ...
            "notes": (
                "Les négociations sont en cours pour établir les règles d'origine spécifiques à ce produit. Les règles génériques du chapitre peuvent s'appliquer en attendant."
                if lang == "fr"
                else "Negotiations are ongoing to establish product-specific rules of origin. Chapter-level generic rules may apply in the interim."
            ),
    ...
    return {
        ...
        "regional_content": 40,
        "notes": (
            "Règles d'origine non définies pour ce code"
            if lang == "fr"
            else "Rules of origin not defined for this code"
        ),
        "source": "UNKNOWN",
