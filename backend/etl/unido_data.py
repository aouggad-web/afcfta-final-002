...
def get_isic4_breakdown(country_iso3: str) -> Optional[Dict]:
    """
    Désagrégation indicative des SEULS secteurs manufacturiers classés parmi
    les "top_sectors" d'un pays (les secteurs les plus significatifs déjà
    retenus dans UNIDO_INDUSTRY_DATA, pas l'ensemble des divisions ISIC 10-33)
    vers le niveau ISIC Rev.4 4 chiffres (classe).
    ...
    retenue parmi les top_sectors est répartie de façon égale entre ses
    classes ISIC Rev.4 (4 chiffres), telles que définies par la nomenclature
    officielle des Nations Unies (UNSD ISIC Rev.4). Ceci fournit une
    estimation de premier niveau de la structure fine des secteurs
    principaux, PAS une structure manufacturière complète (les divisions non
    présentes dans top_sectors sont absentes du résultat), à affiner via une
    intégration ultérieure du détail UNIDO INDSTAT4 4 chiffres par pays.
    """
    ...
    return {
        ...
        "coverage": (
            "Secteurs principaux (top_sectors) uniquement — pas l'ensemble des "
            "divisions manufacturières ISIC 10-33 du pays."
        ),
        "methodology": (
            "Répartition indicative à parts égales des divisions ISIC 2 chiffres "
            "UNIDO INDSTAT4 (limitées aux top_sectors du pays) entre leurs classes "
            "ISIC Rev.4 4 chiffres (nomenclature UNSD). Valeurs à considérer comme "
            "des estimations de structure, en attendant l'intégration du détail "
            "UNIDO INDSTAT4 4 chiffres par pays."
        ),
