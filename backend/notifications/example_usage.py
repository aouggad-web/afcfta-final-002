...
async def example_crawl_lifecycle():
    ...
    await manager.notify_crawl_start(
        job_id=job_id, country_code=country_code, country_name=country_name
    )
...
async def example_custom_notification():
    ...
    from backend.notifications import NotificationSeverity, NotificationType
...
async def example_batch_notifications():
    ...
    for code, name in countries:
        ...
        # Start notification
        await manager.notify_crawl_start(job_id=job_id, country_code=code, country_name=name)
