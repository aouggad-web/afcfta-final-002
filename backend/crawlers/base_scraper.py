...
class BaseScraper(ABC):
    ...
    def __init__(
        ...
        self._rate_limiter = RateLimiter(
            calls=self._config.rate_limit_calls, period=self._config.rate_limit_period
        )
    ...
    async def fetch(
        ...
        while retry_count <= self._config.max_retries:
            ...
            if retry_count < self._config.max_retries:
                ...
                logger.info(
                    f"Retrying in {delay}s (attempt {retry_count + 1}/{self._config.max_retries})"
                )
    ...
    async def fetch_text(self, url: str, method: str = "GET", **kwargs) -> str:
        """
    ...
    async def run(self) -> ScraperResult:
        ...
        result = ScraperResult(country_code=self.country_code, success=False)
