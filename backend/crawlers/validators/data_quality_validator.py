...
class DataQualityValidator(BaseValidator):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.min_completeness = (
            config.get("min_completeness", self.DEFAULT_MIN_COMPLETENESS)
            if config
            else self.DEFAULT_MIN_COMPLETENESS
        )
        self.min_coverage = (
            config.get("min_coverage", self.DEFAULT_MIN_COVERAGE)
            if config
            else self.DEFAULT_MIN_COVERAGE
        )
        self.max_age_days = (
            config.get("max_age_days", self.DEFAULT_MAX_AGE_DAYS)
            if config
            else self.DEFAULT_MAX_AGE_DAYS
        )
        self.outlier_threshold = (
            config.get("outlier_threshold", self.DEFAULT_OUTLIER_STDDEV)
            if config
            else self.DEFAULT_OUTLIER_STDDEV
        )
        self.required_fields = (
            config.get("required_fields", ["hs_code", "rate", "description", "country_code"])
            if config
            else ["hs_code", "rate", "description", "country_code"]
        )
    ...
    def _check_outliers(self, records: List[Dict[str, Any]]):
        ...
        if outliers_found:
            if len(outliers_found) > len(records) * 0.1:  # More than 10% of records
                self._mark_failed(check_name)
                self._add_error(f"Too many outliers detected: {len(outliers_found)} values")
            else:
                self._mark_passed(check_name)
        else:
            self._mark_passed(check_name)
    ...
    def _check_consistency(self, records: List[Dict[str, Any]]):
        ...
        if inconsistencies:
            self._mark_failed(check_name)
            self._add_warning(f"Schema inconsistencies detected in {len(inconsistencies)} records")
        else:
            self._mark_passed(check_name)
        ...
        if "rate" in self.numeric_fields:
            ...
            if rates:
                ...
                if unique_rates == 1:
                    self._mark_failed(check_name)
                    self._add_warning(
                        f"All {len(rates)} tariff rates are identical: {rates[0]}%", field="rate"
                    )
    ...
    def _calculate_quality_metrics(
        ...
        """
        metrics = {"total_records": len(records), "timestamp": datetime.utcnow().isoformat()}

        # Completeness metrics
        if records:
            completeness_scores = [
                self.calculate_completeness(r, self.required_fields) for r in records
            ]
        ...
        metrics["coverage_status"] = (
            "sufficient" if len(records) >= self.min_coverage else "insufficient"
        )
        metrics["coverage_percentage"] = (
            (len(records) / self.min_coverage) * 100 if self.min_coverage > 0 else 100
        )
...
async def validate_data_quality(
    data: Union[Dict[str, Any], List[Dict[str, Any]]], config: Optional[Dict[str, Any]] = None
) -> ValidationResult:
    """
