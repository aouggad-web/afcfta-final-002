...
class BaseValidator(ABC):
    ...
    def _add_issue(
        ...
        issue = ValidationIssue(
            severity=severity, message=message, field=field, value=value, expected=expected
        )
    ...
    def _build_result(
        ...
        """
        errors = [
            issue.message for issue in self._issues if issue.severity == ValidationSeverity.ERROR
        ]
        warnings = [
            issue.message for issue in self._issues if issue.severity == ValidationSeverity.WARNING
        ]
