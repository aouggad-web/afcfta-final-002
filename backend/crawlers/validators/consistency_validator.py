...
class ConsistencyValidator(BaseValidator):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.regional_groups = (
            config.get("regional_groups", self.REGIONAL_GROUPS) if config else self.REGIONAL_GROUPS
        )
        self.max_rate_change = (
            config.get("max_rate_change", self.MAX_RATE_CHANGE_PERCENTAGE)
            if config
            else self.MAX_RATE_CHANGE_PERCENTAGE
        )
    ...
    def _check_internal_consistency(self, records: List[Dict[str, Any]]):
        ...
        inconsistent_hs = [hs for hs, descs in hs_descriptions.items() if len(descs) > 1]
    ...
    def _check_historical_consistency(
        self, records: List[Dict[str, Any]], country_code: Optional[str], metadata: Dict[str, Any]
    ):
        ...
        if "rates" in historical:
            for hs_code, hist_rate in historical["rates"].items():
                if hs_code in current_rates:
                    ...
                    if (
                        percent_change > self.max_rate_change
                        or absolute_change > self.MAX_ABSOLUTE_RATE_CHANGE
                    ):
        ...
        if significant_changes:
            ...
            for change in sorted(
                significant_changes, key=lambda x: x["percent_change"], reverse=True
            )[:5]:
                self._add_warning(
    ...
    def _check_regional_consistency(self, records: List[Dict[str, Any]], country_code: str):
            ...
            if "rate" in record and record["rate"] is not None:
                try:
                    ...
                    if rate < expected_range[0] or rate > expected_range[1]:
                        out_of_range.append(
                            {"hs_code": record.get("hs_code", "unknown"), "rate": rate}
                        )
                except (ValueError, TypeError):
                    continue
        ...
        if out_of_range:
            ...
            if percentage > 10:  # More than 10% out of range
                ...
                self._mark_passed(check_name)
                self._add_warning(
                    f"{len(out_of_range)} rates outside expected regional range", field="rate"
                )
        else:
            self._mark_passed(check_name)

    def _check_reference_consistency(
        self, records: List[Dict[str, Any]], country_code: Optional[str]
    ):
        ...
        if mismatches:
            ...
            if percentage > 5:  # More than 5% mismatches
                ...
                self._mark_passed(check_name)
                self._add_warning(
                    f"{len(mismatches)} records differ from reference data", field="rate"
                )
        else:
            self._mark_passed(check_name)
        ...
        if country_code:
            ...
            if region:
                ...
                metrics["expected_rate_range"] = self.EXPECTED_RATE_RANGES.get(
                    region, self.EXPECTED_RATE_RANGES["OTHER"]
                )
