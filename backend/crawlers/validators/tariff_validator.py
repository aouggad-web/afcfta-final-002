...
class TariffValidator(BaseValidator):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.max_rate = (
            config.get("max_rate", self.MAX_TARIFF_RATE) if config else self.MAX_TARIFF_RATE
        )
        self.required_fields = (
            config.get("required_fields", ["hs_code", "rate", "description"])
            if config
            else ["hs_code", "rate", "description"]
        )
    ...
    def _validate_record(self, record: Dict[str, Any], index: int) -> bool:
        ...
        if "hs_code" in record and record["hs_code"]:
            ...
            if not hs_valid:
                self._mark_failed(check_name)
                self._add_error(
                    f"Record {index}: {hs_message}", field="hs_code", value=record["hs_code"]
                )
                record_valid = False
            else:
                self._mark_passed(check_name)
        ...
        if "rate" in record:
            ...
            if not rate_valid:
                self._mark_failed(check_name)
                self._add_error(
                    f"Record {index}: {rate_message}", field="rate", value=record["rate"]
                )
                record_valid = False
            else:
                self._mark_passed(check_name)
        ...
        if "unit" in record and record["unit"]:
            ...
            if not self.is_valid_string(record["unit"], min_length=1, max_length=50):
                self._mark_failed(check_name)
                self._add_warning(f"Record {index}: Invalid unit of measurement", field="unit")
            else:
                self._mark_passed(check_name)
    ...
    def _validate_tariff_rate(self, rate: Any) -> tuple[bool, Optional[str]]:
        ...
        if rate > self.SUSPICIOUS_TARIFF_RATE:
            return (
                True,
                f"High tariff rate detected: {rate}% (threshold: {self.SUSPICIOUS_TARIFF_RATE}%)",
            )
    ...
    def _validate_field_types(self, record: Dict[str, Any]) -> Dict[str, str]:
        ...
        if "hs_code" in record and record["hs_code"] is not None:
            if not isinstance(record["hs_code"], (str, int, float)):
                errors["hs_code"] = (
                    f"Expected str/int/float, got {type(record['hs_code']).__name__}"
                )
        ...
        if "description" in record and record["description"] is not None:
            if not isinstance(record["description"], str):
                errors["description"] = (
                    f"Expected string, got {type(record['description']).__name__}"
                )
