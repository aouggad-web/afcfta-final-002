...
TAX_COLUMN_MAP = {
    ...
    "TAR_T05": {
        "code": "TSB_PT",
        "name": "Taxe Spéciale Boissons / Prélèvement Transformation",
        "type": "ad_valorem",
    },
...
def download_tariff_excel(output_path: str = "/tmp/guce_tariff.xls") -> str:
    ...
    resp = requests.get(
        DOWNLOAD_URL, params={"lang": "fr"}, timeout=120, headers={"User-Agent": "Mozilla/5.0"}
    )
        ...
        taxes_detail.append(
            {
                ...
                "base": (
                    "CIF"
                    if tax_info["code"] in ("DD", "TVA", "TUB", "TSB_PT", "TUF")
                    else "variable"
                ),
            }
        )
