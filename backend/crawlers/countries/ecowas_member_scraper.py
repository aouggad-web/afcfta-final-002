...
COUNTRY_CONFIGS = {
    "BEN": {
        ...
        "national_taxes": {
            ...
            "PCS": {
                "code": "PCS",
                "name": "Prélèvement Communautaire de Solidarité (UEMOA)",
                "rate": 1.0,
                "type": "ad_valorem",
                "base": "CIF",
            },
            "PCC": {
                "code": "PCC",
                "name": "Prélèvement Communautaire CEDEAO",
                "rate": 0.5,
                "type": "ad_valorem",
                "base": "CIF",
            },
    ...
    "BFA": {
        ...
        "national_taxes": {
            ...
            "PC_AES": {
                "code": "PC-AES",
                "name": "Prélèvement Confédéral AES",
                "rate": 0.5,
                "type": "ad_valorem",
                "base": "CIF",
            },
...
def download_ecowas_nomenclature(output_path: str = "/tmp/guce_tariff.xls") -> str:
    ...
    resp = requests.get(
        GUCE_DOWNLOAD_URL, params={"lang": "fr"}, timeout=120, headers={"User-Agent": "Mozilla/5.0"}
    )
...
def build_country_taxes(
    ...
    if country_code not in TIC_COUNTRY_EXEMPT and chapter in TIC_CHAPTERS_KNOWN:
        taxes_detail.append(
            {
                "tax_code": "TIC",
                ...
                "rate_type": "ad_valorem",
                "base": "CIF",
                "note": "TIC applicable — taux non crawlé. Vérifier CGI national (Loi de Finances).",
                "source_gap": True,
            }
        )
...
def generate_country_tariffs(country_code: str, xls_path: str) -> List[dict]:
    ...
    logger.info(
        f"  {config['country_name']}: {stats['parsed']} positions, {stats['chapters']} chapters"
    )
    return positions
