        ...
        try:
            ...
            return {
                ...
                "data": {
                    ...
                    "lines_with_sub_positions": tariff_data.get("summary", {}).get(
    ...
    async def save_to_db(self, data: Dict[str, Any]) -> int:
        ...
        try:
            ...
            result = await collection.update_one(
                {"country_code": self.country_code}, {"$set": doc}, upsert=True
            )
...
class ScraperFactory:
    ...
    def register(cls, country_code: str, scraper_class: Type[BaseScraper]):
        ...
        if country_code not in AFRICAN_COUNTRIES_REGISTRY:
            logger.warning(f"Registering scraper for unknown country code: {country_code}")

        cls._registry[country_code] = scraper_class
        logger.info(f"Registered {scraper_class.__name__} for {country_code}")
    ...
    def get_scraper(
        ...
        if scraper_class == cls._default_scraper:
            logger.info(f"No specific scraper for {country_code}, using generic scraper")

        return scraper_class(country_code=country_code, db_client=db_client, config=config)
    ...
    def list_registered_scrapers(cls) -> Dict[str, str]:
        ...
        """
        return {code: scraper_class.__name__ for code, scraper_class in cls._registry.items()}
    ...
    def get_registry_stats(cls) -> Dict[str, Any]:
        ...
        return {
            ...
            "coverage_percentage": (
                (registered / total_countries * 100) if total_countries > 0 else 0
            ),
            ...
            try:
                ...
                for attr_name in dir(module):
                    ...
                    if (
                        ...
                        and attr not in (BaseScraper, GenericScraper)
                        and hasattr(attr, "_country_code")
                    ):
