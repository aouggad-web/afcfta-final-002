...
class UNCTADMaritimeCollector:
    ...
    UNCTAD_LSCI_URL = (
        "https://datacatalogfiles.worldbank.org/ddh-published/0037800/DR0046325/LSCI.csv"
    )
    UNCTAD_PORT_CALLS_URL = "https://unctadstat.unctad.org/datacentre/dataviewer/US.PortCalls"

    def __init__(self):
        self.session = requests.Session()
        self.session.headers.update({"User-Agent": "ZLECAf-Logistics-App/1.0"})
                ...
                port["latest_stats"].update(
                    {
                        ...
                        "berth_productivity_moves_per_hour": perf[
                            "berth_productivity_moves_per_hour"
                        ],
...
def main():
    ...
    collector.enrich_ports_with_unctad_data(
        ports_file=ports_file, output_file=ports_file  # Update in-place to avoid multiple versions
    )
