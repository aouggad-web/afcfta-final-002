...
def _resolve_compare_countries(
    _, __, countries: List[str], sector: str = "general"
) -> Dict[str, Any]:
    from intelligence.ai_engine.investment_scoring import get_intelligence_engine
...
def _resolve_compare_regions(
    _, __, regions: List[str], metrics: Optional[List[str]] = None
) -> Dict[str, Any]:
...
def _resolve_risk_assessment(
    _, __, country: str, sector: str = "general", investmentSize: str = "medium"
) -> Dict[str, Any]:
    from intelligence.ai_engine.investment_scoring import get_intelligence_engine
...
def _mutate_warm_cache(_, __) -> Dict[str, Any]:
    return {
        "status": "Cache warming initiated (async)",
...
_RESOLVER_MAP = {
    ...
    "predictTradeFlows": lambda p: _resolve_predict_trade(
        ...
        None,
        p.get("originCountry", ""),
        p.get("destinationCountry", ""),
        p.get("productCategory", ""),
        p.get("timeframeMonths", 12),
    ),
...
_MUTATION_MAP = {
    "saveSearch": lambda p: _mutate_save_search(
        None, None, p.get("searchParams", {}), p.get("name", "")
    ),
    "createInvestmentAlert": lambda p: _mutate_create_alert(
        None, None, p.get("criteria", {}), p.get("userId", "")
    ),
