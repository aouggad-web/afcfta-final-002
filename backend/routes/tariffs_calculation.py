...
async def get_detailed_tariff(country_code: str, hs_code: str, language: str = Query(default="fr")):
    ...
    return {
        ...
        "vat_rate": vat_rate,
        "description": (
            country_data.get("description_fr" if language == "fr" else "description_en")
            if country_data
            else f"Code HS {hs6}"
        ),
...
async def get_sub_position_tariff(
    country_code: str, full_code: str, language: str = Query(default="fr")
):
...
async def get_sub_positions(country_code: str, hs6_code: str, language: str = Query(default="fr")):
