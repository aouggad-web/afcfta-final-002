...
async def cache_health():
    ...
    try:
        client.ping()
        return {"healthy": True, "message": "Redis is responding"}
...
async def clear_all_cache():
    ...
    return {"success": True, "keys_deleted": deleted, "message": f"Cleared {deleted} cached keys"}
...
async def list_cache_keys(pattern: str = "*", limit: int = 100):
    ...
    try:
        ...
        for key in keys:
            ttl = client.ttl(key)
            keys_with_ttl.append({"key": key, "ttl_seconds": ttl if ttl > 0 else "no expiry"})

        return {"total_matching": len(keys), "showing": len(keys_with_ttl), "keys": keys_with_ttl}
    except Exception as e:
