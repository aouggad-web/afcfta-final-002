...
try:
    _spec = importlib.util.spec_from_file_location(
        "engine_api_engine_service", _engine_service_path
    )
...
async def get_available_countries():
    ...
    return CountriesResponse(success=True, countries=countries, total=len(countries))
...
async def get_tariff_details(
    ...
    if search_type == "hs6":
        ...
        if not results or not results[0].success:
            ...
            return DetailsResponse(
                success=False, country_iso3=country, code=code, hs6=code, error=error_msg
            )
    ...
    if not result.success:
        return DetailsResponse(
            success=False, country_iso3=country, code=code, hs6=result.hs6, error=result.error
        )
...
async def get_all_sub_positions(
    ...
    for result in results:
        if result.success and result.data:
            ...
            responses.append(
                DetailsResponse(
                    success=False, country_iso3=country, code=hs6, hs6=hs6, error=result.error
                )
            )
