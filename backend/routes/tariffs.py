...
async def get_hs6_tariffs_chapter_endpoint(chapter: str, language: str = Query("fr")):
...
async def get_country_tariffs_endpoint(
    country_code: str, hs_code: str = Query("18", description="HS code (2-6 digits)")
):
    """
    ...
    return {
        ...
        "tariffs": {
            ...
            "potential_savings_pct": (
                round((npf_rate - zlecaf_rate) / npf_rate * 100, 1) if npf_rate > 0 else 0
            ),
        },
        "taxes": {
            "vat_rate": vat_rate,
        ...
        "sources": {"tariff": npf_source, "zlecaf": zlecaf_source, "vat": vat_source},
        ...
        results.append(
            {
                ...
                "vat_rate_pct": f"{vat_rate * 100:.1f}%",
                "other_taxes_pct": f"{other_rate * 100:.1f}%",
                "total_cost_factor_npf": round(
                    1 + npf_rate + vat_rate * (1 + npf_rate) + other_rate, 3
                ),
                "total_cost_factor_zlecaf": round(
                    1 + zlecaf_rate + vat_rate * (1 + zlecaf_rate) + other_rate, 3
                ),
            }
        )
...
async def get_detailed_tariff_endpoint(
    country_code: str, hs_code: str, language: str = Query("fr")
):
    """
    ...
    if not result:
        raise HTTPException(status_code=404, detail=f"Tarif non trouvé pour {iso3}/{hs_code}")

    return result


@router.get("/tariffs/sub-position/{country_code}/{full_code}")
async def get_sub_position_rate_endpoint(country_code: str, full_code: str):
    ...
    if rate is None:
        raise HTTPException(
            status_code=404, detail=f"Sous-position {full_code} non trouvée pour {iso3}"
        )
