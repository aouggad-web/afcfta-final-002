    ...
    if _corridors_data is None:
        try:
            ...
            try:
                ...
                enh_index = {
                    c["corridor_id"]: c
                    for c in enh_data.get("enhanced_corridors", [])
                    if "corridor_id" in c
                }
...
def get_all_corridors(
    corridor_type: Optional[str] = None, importance: Optional[str] = None
) -> List[Dict]:
    """
...
def get_corridors_by_country(country_iso: str) -> List[Dict]:
    ...
    corridors = load_corridors_data()
    return [c for c in corridors if country_iso in c.get("countries_spanned", [])]
...
def get_corridors_statistics() -> Dict:
    ...
    total_freight = sum(
        c.get("stats", {}).get("freight_throughput_tons", 0) for c in corridors if c.get("stats")
    )
    ...
    trucking_companies = len(
        [o for o in all_operators if o.get("operator_type") == "trucking_company"]
    )
