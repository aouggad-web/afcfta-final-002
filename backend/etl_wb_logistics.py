...
class WorldBankLogisticsCollector:
    ...
    def fetch_indicator(
        self, country_iso3: str, indicator_code: str, start_year: int = 2015, end_year: int = 2024
    ) -> List[Dict]:
        """
        ...
        for indicator_name, indicator_code in LOGISTICS_INDICATORS.items():
            ...
            for entry in data:
                if entry.get("value") is not None:
                    values.append({"year": entry.get("date"), "value": entry.get("value")})
...
def main():
    ...
    data = collector.collect_all_africa(output_file="/app/wb_logistics_africa.json")
