...
# Air freight pricing model constants (USD)
AIR_DISTANCE_BANDS = [
    {"max_km": 1000, "rate_usd_per_kg": 1.85},
    {"max_km": 3000, "rate_usd_per_kg": 2.55},
    {"max_km": 6000, "rate_usd_per_kg": 3.35},
    {"max_km": float("inf"), "rate_usd_per_kg": 4.15},
]
...
SECURITY_FEE_USD_PER_KG = 0.06
TERMINAL_HANDLING_USD_PER_KG = 0.11
TERMINAL_HANDLING_MIN_USD = 55.0
DOCUMENTATION_FEE_USD = 35.0
VOLUMETRIC_FACTOR_AIR_KG_PER_M3 = 167

def _load_enhanced_airport_index():
    """Load enhanced airport data indexed by airport_id."""
    ...
    try:
        ...
        for airport in enhanced_data.get('enhanced_locations', []):
            airport_id = airport.get('airport_id')
            if airport_id:
                _enhanced_airport_index[airport_id] = airport
    except (FileNotFoundError, json.JSONDecodeError) as e:
        print(f"⚠️ Could not load enhanced airports file: {e}")
