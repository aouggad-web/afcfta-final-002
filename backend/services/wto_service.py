        ...
        try:
            ...
            if e.response.status_code == 429:  # Rate limit
                if attempt < max_retries - 1:
                    ...
                    logger.warning(
                        f"⚠️ Rate limit hit (429), retrying in {wait_time}s (attempt {attempt + 1}/{max_retries})..."
                    )
                ...
                logger.warning(
                    f"⚠️ Unauthorized (401) for URL: {url}. Check API credentials. Skipping..."
                )
                return None
            else:
            ...
            if attempt < max_retries - 1:
                wait_time = calculate_backoff(attempt)
                logger.warning(
                    f"⚠️ Request timeout, retrying in {wait_time}s (attempt {attempt + 1}/{max_retries})..."
                )
                time.sleep(wait_time)
            else:
...
class WTOService:
    ...
    def get_tariff_data(
        ...
        params = {"i": "IDB_MFN_SMPL", "r": reporter, "p": partner, "fmt": "json"}
    ...
    def get_tariff_data_sync(
        self, reporter_code: str, partner_code: str, product_code: Optional[str] = None
    ) -> Optional[Dict]:
        """
    ...
    async def get_mfn_average(self, country_code: str, product_group: str = None) -> Optional[Dict]:
        """
        ...
        params = {"i": indicator, "r": country_code, "ps": "last", "max": 100}
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.get(f"{self.BASE_URL}/data", params=params, headers=headers)
    ...
    def get_trade_indicators(
        self, country_code: str, indicator: str = "TRADE_VALUE"
    ) -> Optional[Dict]:
        """
