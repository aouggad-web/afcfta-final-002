...
def _summarise(line_items: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    """
    provider_by_ccy: Dict[str, float] = {}
    formality_by_ccy: Dict[str, float] = {}
    for item in line_items:
        ...
        if amount is None or not ccy:
            continue
        bucket = provider_by_ccy if item["scope"] == "provider" else formality_by_ccy
        bucket[ccy] = round(bucket.get(ccy, 0.0) + float(amount), 2)
    ...
    all_ccys = set(provider_by_ccy) | set(formality_by_ccy)
    if len(all_ccys) == 1:
        ccy = next(iter(all_ccys))
        regulatory_total = round(provider_by_ccy.get(ccy, 0.0) + formality_by_ccy.get(ccy, 0.0), 2)
    ...
    return {
        ...
        "provider_fees_total": provider_by_ccy or None,
        "formality_fees_total": formality_by_ccy or None,
