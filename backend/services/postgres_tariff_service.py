...
class PostgresTariffService:
    ...
    def get_sub_positions(self, country_iso3: str, hs6: str, language: str = "fr") -> List[Dict]:
        ...
        for r in results:
            desc = (
                r["description_fr"]
                if language == "fr"
                else (r["description_en"] or r["description_fr"])
            )
            formatted.append(
                {
                    "code": r["code"],
    ...
    def search_commodities(
        self, country_iso3: str, query: str, limit: int = 50, language: str = "fr"
    ) -> List[Dict]:
        ...
        results = self._execute_query(
            search_query, {"iso3": country_iso3.upper(), "query": query, "limit": limit}
        )
    ...
    def calculate_tariffs(self, country_iso3: str, hs6: str, value: float = 1000) -> Dict:
        ...
        if not sub_positions:
            return {"success": False, "error": f"No data found for {country_iso3}/{hs6}"}
        ...
        total_npf = (
            sum(c["npf_duty"] for c in calculations) / len(calculations) if calculations else 0
        )
        total_zlecaf = (
            sum(c["zlecaf_duty"] for c in calculations) / len(calculations) if calculations else 0
        )
