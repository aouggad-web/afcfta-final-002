...
try:
    ...
    logging.warning("emergentintegrations package not installed; AI features will be disabled")
...
class ClaudeTradeService:
    ...
    def __init__(self):
        # Use the Emergent LLM Universal Key which supports Anthropic Claude.
        # Falls back to ANTHROPIC_API_KEY for environments using a native Anthropic key.
        self.api_key = os.environ.get("EMERGENT_LLM_KEY") or os.environ.get("ANTHROPIC_API_KEY")
        if not self.api_key:
            logger.warning("Neither EMERGENT_LLM_KEY nor ANTHROPIC_API_KEY found; AI features disabled")
        if not ANTHROPIC_AVAILABLE:
            logger.warning("emergentintegrations package not installed; AI features disabled")
    ...
    async def _call_claude(self, user_prompt: str, max_tokens: int = 8192) -> str:
        """Call Claude via emergentintegrations and return raw text."""
        if not self._is_ready():
            raise RuntimeError(
                "EMERGENT_LLM_KEY is not set or emergentintegrations package not installed."
            )
        self._session_counter += 1
        session_id = f"claude-trade-{self._session_counter}"
        chat = LlmChat(
        ...
        if isinstance(response, str):
            return response
        # Defensive: handle objects with a .text attribute just in case
    ...
    async def analyze_trade_opportunities(
        ...
        if not self._is_ready():
            return {"error": "EMERGENT_LLM_KEY not configured", "opportunities": []}
