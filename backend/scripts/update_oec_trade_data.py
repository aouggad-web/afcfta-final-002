...
from services.oec_trade_service import AFRICAN_COUNTRIES_OEC, oec_service
    ...
    for idx, row in df.iterrows():
        ...
        if iso3 in oec_data:
            ...
            if old_exports != new_exports or old_imports != new_imports:
                print(
                    f"{code}: Exports {old_exports}B → {new_exports}B, Imports {old_imports}B → {new_imports}B"
                )
                updates_made += 1
