...
def generate_demo_data(output_dir: Path):
    ...
    tariff_lines = {
        ...
        "countries": {
            "MA": {
                "name": "Maroc",
                "total_lines": 6127,
                "categories": {"A": 4500, "B": 1200, "C": 427},
            },
            "ZA": {
                "name": "Afrique du Sud",
                "total_lines": 7852,
                "categories": {"A": 6000, "B": 1500, "C": 352},
            },
            "CM": {
                "name": "Cameroun",
                "total_lines": 5800,
                "categories": {"A": 3800, "B": 1400, "C": 600},
            },
            "CF": {
                "name": "République Centrafricaine",
                "total_lines": 4200,
                "categories": {"A": 2800, "B": 1000, "C": 400},
            },
            "TD": {
                "name": "Tchad",
                "total_lines": 4500,
                "categories": {"A": 3000, "B": 1100, "C": 400},
            },
            "CG": {
                "name": "République du Congo",
                "total_lines": 4800,
                "categories": {"A": 3200, "B": 1200, "C": 400},
            },
            "GQ": {
                "name": "Guinée Équatoriale",
                "total_lines": 3500,
                "categories": {"A": 2300, "B": 850, "C": 350},
            },
            "GA": {
                "name": "Gabon",
                "total_lines": 5200,
                "categories": {"A": 3500, "B": 1200, "C": 500},
            },
        },
    }
    ...
    rules_of_origin = {
        "metadata": {"version": "1.0", "source": "ZLECAf Annex 2"},
        "rules": {
            "01": {
                "chapter": "01",
    ...
    with open(
        output_dir / "zlecaf_dismantling_schedule.csv", "w", encoding="utf-8", newline=""
    ) as f:
        writer = csv.writer(f)
    ...
    tariff_origin_phase = {
        "metadata": {"description": "Integrated tariff, origin rules, and phase-out schedule"},
        "data": {
            "MA": {
                "84": {
                    "category": "A",
                    "rule": "40% regional content",
                    "phases": {"2024": 5.0, "2029": 2.0, "2034": 0.0},
                }
            }
        },
    }
    ...
    with open(
        output_dir / "zlecaf_africa_vs_world_tariffs.csv", "w", encoding="utf-8", newline=""
    ) as f:
        writer = csv.writer(f)
        writer.writerows(
            [
...
def _fetch_unctad_trains(api_url: str, api_key: str, output_dir: Path) -> bool:
    ...
    try:
        ...
        logger.info(
            "UNCTAD TRAINS: données reçues (%d éléments)", len(data.get("tariff_lines", []))
        )
        ...
        logger.warning(
            "UNCTAD TRAINS indisponible (%s); utilisation des données de démonstration.", exc
        )
        return False
...
def _fetch_etariff(api_url: str, api_key: str, output_dir: Path) -> bool:
    ...
    try:
        ...
        logger.warning(
            "e-Tariff Portal indisponible (%s); utilisation des données de démonstration.", exc
        )
        return False
...
def main():
    ...
    parser.add_argument(
        "--demo", action="store_true", help="Mode démonstration avec données simulées"
    )
    parser.add_argument(
        "--output",
        type=str,
