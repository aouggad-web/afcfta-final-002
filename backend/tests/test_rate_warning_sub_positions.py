...
class TestRateWarningFeature:
    ...
    def test_rice_100630_to_nigeria_has_varying_rates(self):
        ...
        response = requests.post(
            f"{BASE_URL}/api/calculate-tariff",
            json={
        ...
        if rate_warning:
            ...
            print(
                f"Rice 100630 to Nigeria - Min rate: {min_rate*100:.1f}%, Max rate: {max_rate*100:.1f}%"
            )
            ...
            print(
                f"✅ Rice 100630 rate_warning verified: {min_rate*100:.1f}% - {max_rate*100:.1f}%"
            )
        else:
            print("⚠️ rate_warning is None - checking if sub_positions exist")
            # Check if has_varying_sub_positions is True
            assert (
                data.get("has_varying_sub_positions") == True
            ), "has_varying_sub_positions should be True for rice"
    ...
    def test_rice_100630_sub_positions_details(self):
        ...
        if sub_positions:
            ...
            for sp in sub_positions:
                ...
                assert (
                    "description_fr" in sp or "description_en" in sp
                ), "Each sub-position should have description"

            print(f"✅ Found {len(sub_positions)} sub-positions for rice 100630:")
            for sp in sub_positions:
                print(
                    f"   - {sp.get('code')}: {sp.get('dd_rate_pct', sp.get('dd_rate')*100 if sp.get('dd_rate') else 'N/A')} - {sp.get('description_fr', sp.get('description_en', ''))}"
                )
        else:
    ...
    def test_cars_870323_to_nigeria_has_varying_rates(self):
        ...
        if rate_warning:
            assert (
                rate_warning.get("has_variation") == True
            ), "has_variation should be True for cars"

            min_rate = rate_warning.get("min_rate", 0)
            max_rate = rate_warning.get("max_rate", 0)

            print(
                f"Cars 870323 to Nigeria - Min rate: {min_rate*100:.1f}%, Max rate: {max_rate*100:.1f}%"
            )
            ...
            print(
                f"✅ Cars 870323 rate_warning verified: {min_rate*100:.1f}% - {max_rate*100:.1f}%"
            )
        else:
            # Check if has_varying_sub_positions is True
    ...
    def test_cars_870323_sub_positions_details(self):
        ...
        if sub_positions:
            ...
            for sp in sub_positions:
                print(
                    f"   - {sp.get('code')}: {sp.get('dd_rate_pct', '')} - {sp.get('description_fr', sp.get('description_en', ''))}"
                )
        else:
            count = data.get("available_sub_positions_count", 0)
            print(f"⚠️ sub_positions_details is None, available_sub_positions_count: {count}")
...
class TestNoWarningForUniformRates:
    ...
    def test_medicines_300490_no_warning(self):
        ...
        if rate_warning:
            ...
            assert (
                has_variation == False
            ), f"Medicines should NOT have varying rates, got has_variation={has_variation}"
    ...
    def test_kenya_rice_varying_rates(self):
        ...
        if rate_warning:
            ...
            if rate_warning.get("has_variation"):
                print(
                    f"   Rate range: {rate_warning.get('min_rate_pct')} - {rate_warning.get('max_rate_pct')}"
                )
...
class TestEdgeCases:
    ...
    def test_product_without_sub_positions_data(self):
        ...
        if rate_warning:
            assert (
                rate_warning.get("has_variation") == False
            ), "Non-existent code should not have varying rates"
