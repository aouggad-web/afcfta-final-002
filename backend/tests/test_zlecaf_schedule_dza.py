...
def test_list_b_standard_partner_full_rate_during_transition():
    ...
    rate, source = compute_dza_zlecaf_rate(
        code, "TUN", 0.30, as_of=datetime.date(2024, 6, 18)
    )
    assert rate == 0.30  # transition 2021-2025 : droit commun maintenu
...
def test_list_b_standard_partner_reduction_starts_2026():
    ...
    rate, _ = compute_dza_zlecaf_rate(
        code, "TUN", 0.30, as_of=datetime.date(2026, 6, 18)
    )
    assert abs(rate - 0.24) < 1e-9  # 80% du droit de base
