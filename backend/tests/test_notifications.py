...
import pytest
from notifications.base_notifier import NotificationSeverity, NotificationType
...
async def test_crawl_started_notification():
    ...
    result = await manager.notify_crawl_start(
        job_id="test_job_001", country_code="MA", country_name="Morocco"
    )
...
async def test_crawl_success_notification():
    ...
    manager = NotificationManager()
    stats = {"items_scraped": 100, "validation_score": 95.5}
...
async def test_multiple_notifications_sequence():
    ...
    result1 = await manager.notify_crawl_start(
        job_id="test_job_seq", country_code="SN", country_name="Senegal"
    )
