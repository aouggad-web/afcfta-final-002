...
class TestCEMACDataFiles:
    ...
    def test_minimum_tariff_line_count(self):
        for cc in CEMAC_CODES:
            ...
            # GNQ (original enhanced_v2) has 5831 hs6-level tariff_lines.
            # CMR/CAF/COG/GAB/TCD (migrated from old 8-digit format) have 4801
            # hs6-level tariff_lines (fewer unique hs6 codes than raw positions).
            assert len(lines) >= 4000, f"{cc}: only {len(lines)} lines (expected ≥4000)"
    ...
    def test_enhanced_v2_dd_rate_in_cemac_bands(self):
        ...
        GNQ is generated by the dedicated CEMAC crawler and strictly enforces
        the 4-band structure.  Other CEMAC members were migrated from legacy
        source data that may contain occasional non-standard rates; we verify
        their rates are numeric and within a reasonable 0–100 % range.
        """
        ...
        for cc in CEMAC_CODES:
            ...
            for line in lines[:200]:  # sample check
                ...
                if dd is None:
                    continue
                if cc == "GNQ":
                    # GNQ was generated by the CEMAC crawler — enforce strict bands
                    assert dd in valid_dd, f"{cc}: unexpected DD rate {dd} (valid: {valid_dd})"
                else:
    ...
    def test_all_cemac_countries_are_enhanced_v2(self):
        ...
        for cc in CEMAC_CODES:
            data = self._load(cc)
            assert (
                data.get("data_format") == "enhanced_v2"
            ), f"{cc}: expected enhanced_v2, got {data.get('data_format')}"

    def test_cemac_enhanced_v2_has_dd_and_vat_fields(self):
        """All CEMAC enhanced_v2 tariff_lines include dd_rate and vat_rate."""
        for cc in CEMAC_CODES:
            data = self._load(cc)
            lines = data.get("tariff_lines", [])
            if not lines:
                continue
            sample = lines[0]
            assert "dd_rate" in sample, f"{cc}: missing dd_rate"
            assert "vat_rate" in sample, f"{cc}: missing vat_rate"
...
class TestCEMACRoutes:
    ...
    def test_data_summary_countries_have_data(self):
        ...
        assert len(available) == 6, f"Expected 6 countries with data, got {len(available)}"
    ...
    def test_data_summary_cmr_has_expected_line_count(self):
        ...
        # CMR is now enhanced_v2: tariff_lines = unique hs6 groups (~4801),
        # plus ~5239 sub-positions from the migrated 8-digit CEMAC codes.
        assert cmr["tariff_lines"] >= 4500
    ...
    def test_country_data_summary_cmr(self):
        ...
        # CMR is now enhanced_v2: 4801 hs6-level tariff_lines + 5239 sub-positions
        assert entry["tariff_lines"] >= 4500
...
class TestCEMACSubPositions:
    ...
    def test_sub_positions_cmr_has_sub_positions(self):
        ...
        result = get_cemac_sub_positions()
        cmr = next(r for r in result["countries"] if r["iso3"] == "CMR")
        assert cmr["sub_positions"] >= 5000
        assert cmr["data_format"] == "enhanced_v2"
    ...
    def test_sub_positions_regional_totals_fields(self):
        ...
        for field in (
            "total_tariff_lines",
            "total_sub_positions",
            "total_lines_with_sub_positions",
