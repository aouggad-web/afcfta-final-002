...
class TestRulesOfOriginWheat:
    ...
    def test_wheat_rule_name_contains_wholly_obtained(self):
        ...
        assert (
            "entièrement" in rule_name or "wholly" in rule_name or "wo" in rule_name.lower()
...
class TestRulesOfOriginMachines:
    ...
    def test_machines_rule_is_cth_or_va60(self):
        ...
        assert (
            primary_code in valid_codes or alt_code in valid_codes
...
class TestRulesOfOriginClothing:
    ...
    def test_clothing_rule_is_cth(self):
        ...
        assert (
            rule_code == "CTH" or "tariff heading" in rule_name or "position tarifaire" in rule_name
...
class TestRulesOfOriginVehicles:
    ...
    def test_vehicles_rule_is_ytb_or_va60(self):
        ...
        assert (
            is_valid
        ), f"Expected YTB or VA60 for vehicles, got primary: {primary_code}, alt: {alt_code}, status: {status}"
...
class TestRulesOfOriginNotGeneric:
    ...
    def test_wheat_not_generic_35_percent(self):
        ...
        assert (
            regional_content != 35 or rule_code == "WO"
        ), f"Wheat should not have generic 35% rule, got: {regional_content}%"
        print(
            f"✅ Wheat is NOT generic 35%: rule={rule_code}, regional_content={regional_content}%"
        )
...
class TestCalculateTariffIncludesRulesOfOrigin:
    ...
    def test_calculate_tariff_coffee_has_wo_rule(self):
        ...
        assert (
            rule_code == "WO" or "wholly" in rule.lower() or "entièrement" in rule.lower()
    ...
    def test_calculate_tariff_clothing_has_cth_rule(self):
        ...
        assert (
            rule_code == "CTH"
            or "tariff heading" in rule.lower()
            or "position tarifaire" in rule.lower()
