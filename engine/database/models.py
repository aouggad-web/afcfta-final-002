...
class Commodity(Base):
    ...
    country_iso3 = Column(
        String(3), ForeignKey("countries.iso3", ondelete="CASCADE"), nullable=False
    )
    ...
    requirements = relationship(
        "Requirement", back_populates="commodity", cascade="all, delete-orphan"
    )
    fiscal_advantages = relationship(
        "FiscalAdvantage", back_populates="commodity", cascade="all, delete-orphan"
    )
...
class Requirement(Base):
    ...
    __table_args__ = (Index("idx_requirement_commodity", "commodity_id"),)
...
class FiscalAdvantage(Base):
    ...
    __table_args__ = (Index("idx_fiscal_advantage_commodity", "commodity_id"),)
...
def get_engine(database_url: str = None):
    ...
    if database_url is None:
        ...
        database_url = os.environ.get(
            "DATABASE_URL", "postgresql://postgres:postgres@localhost:5432/afcfta_regulatory"
        )
