...
def clear_tables(session):
    ...
    session.execute(
        text(
            "TRUNCATE fiscal_advantages, requirements, measures, commodities, countries RESTART IDENTITY CASCADE"
        )
    )
    session.commit()
        ...
        commodities_batch.append(
            {
                ...
                "description_en": (
                    commodity.get("description_en", "")[:2000]
                    if commodity.get("description_en")
                    else None
                ),
        ...
        for r in c.get("requirements", []):
            ...
            """
                ),
                {
                    ...
                    "authority": (
                        r.get("issuing_authority", "")[:200] if r.get("issuing_authority") else None
                    ),
                },
            )
