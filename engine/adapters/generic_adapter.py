...
class GenericAdapter(BaseAdapter):
    ...
    def _extract_measures(self, parent: Dict, sub: Dict = None) -> list:
        ...
        if other_taxes_rate > 0 and not any(
            t.get("tax") not in ["D.D", "T.V.A"] for t in taxes_detail
        ):
            measures.append(
                Measure(
                    country_iso3=self.country_iso3,
    ...
    def _extract_requirements(self, parent: Dict, national_code: str) -> list:
        ...
        for form in formalities:
            ...
            authority = default_authorities.get(
                code, self.config.get("tax_authority", f"Autorité {self.country_iso3}")
            )
