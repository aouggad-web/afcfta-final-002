...
VERIFY_NOTE = (
    "Taxe nationale hors CET — taux standard documenté, à confirmer "
...
EXTRA_NATIONAL: Dict[str, List[dict]] = {
    "KEN": [
        dict(
            ...
            rate_pct=2.5,
            basis=DutyBasis.CUSTOMS_VALUE,
            sequence=30,
            legal_reference="Miscellaneous Fees and Levies Act 2016, " "modifié Finance Act 2023",
...
class EacCetAdapter:
    ...
    SOURCE_NAME = (
        "EAC CET 2022 Version — Common External Tariff "
    ...
    def __init__(
        self, source_path: str, version_date: Optional[date] = None, hs_version: str = "HS2022"
    ):
    ...
    def provenance(self, country: str) -> Provenance:
        notes = (
            "Droits de douane issus du CET communautaire officiel "
    ...
    def parse_source(self) -> List[dict]:
        ...
        for row in reader:
            ...
            if not re.fullmatch(r"\d{8}", code) or rate is None:
                ...
                continue
            sensitive = (
                (row.get(cols["sensitive"], "") or "") == "1" if "sensitive" in cols else False
            )
            ...
            rows.append(
                {
                    ...
                    "specific_unit": (
                        row.get(cols["specific_unit"], "") or None
                        if "specific_unit" in cols
                        else None
                    ),
                    "sensitive": sensitive,
    ...
    def _measures(self, country: str, row: dict) -> List[Measure]:
        ...
        def add(**kw):
            ...
            defaults.update(kw)
            if defaults.get("rate_type") == RateType.AD_VALOREM and defaults.get("rate_pct") in (
                None,
                0.0,
            ):
        ...
        if row["specific"] is not None:
            dd_kw.update(
                rate_type=RateType.ALTERNATIVE,
        ...
        if country in VAT_BY_COUNTRY:
            ...
            upstream = [
                m.code
                for m in measures
                if m.basis in (DutyBasis.CIF, DutyBasis.CUSTOMS_VALUE, DutyBasis.CIF_PLUS_INCLUDED)
            ]
            add(
                ...
                basis_includes=upstream,
                observation=VERIFY_NOTE + " ; assiette = CAF + droits et taxes en amont (hors TVA)",
            )
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--version-date", default=None, help="Millésime du CET (YYYY-MM-DD, défaut 2022-07-01)"
    )
    args = ap.parse_args()
    stats = run(args.source, args.output_dir, args.countries, args.version_date)
    print(f"EAC CET -> {args.output_dir}")
    print(
        f"  lignes source: {stats['source_lines']} | ignorées: {stats['skipped']} "
