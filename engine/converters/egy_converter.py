...
_EGY_AUTHORITIES = [
    ...
    (re.compile(r"بيئ|environ", re.IGNORECASE),
     ("Ministère de l'Environnement", "MOE_EGY")),
    (re.compile(r"صحة.*نبات|phyto", re.IGNORECASE),
     ("Service de la Quarantaine Phytosanitaire", "PLANT_QUAR")),
    (re.compile(r"cites|أنواع.*مهددة", re.IGNORECASE),
     ("Autorité CITES Égypte", "CITES_EGY")),
    (re.compile(r"جمارك|douane|custom", re.IGNORECASE),
    ...
    (re.compile(r"صحة\b|santé\b|health\b", re.IGNORECASE),
     ("Ministère de la Santé", "MOH_EGY")),
]
...
def _parse_instructions(instructions: list[str], code_nat: str,
                         dd_rate: Optional[float]) -> tuple[list[Requirement], list[FiscalAdvantage]]:
    """
