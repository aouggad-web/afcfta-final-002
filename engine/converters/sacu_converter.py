...
def _provenance(country: str) -> Provenance:
    return Provenance(
        ...
        notes=(
            f"Crawl sars.gov.za — TEC SACU commun ; TVA {country} = "
...
def convert_country(country: str, output_path: Optional[Path] = None) -> int:
    ...
    for pos in positions:
        ...
        for tax in pos.get("taxes") or []:
            ...
            duty = parse_duty_value(str(tax.get("raw_value") or ""), rate_hint=tax.get("rate_pct"))
                ...
                if duty["rate_pct"] is not None and duty["rate_pct"] < (dd_rate or 999):
                    advantages.append(
                        FiscalAdvantage(
                            ...
                            tax_code="DD",
                            reduced_rate_pct=(
                                duty["rate_pct"] if duty["rate_pct"] is not None else 0.0
                            ),
