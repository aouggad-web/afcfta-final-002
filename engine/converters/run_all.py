...
_REGISTRY: dict[str, tuple] = {
    ...
    # SACU
    "ZAF": (
        lambda: sacu_converter.convert_country("ZAF"),
        "Afr. du Sud — sars.gov.za",
        "8 589 pos.",
    ),
    ...
    # CEDEAO
    "BEN": (
        lambda: ecowas_converter.convert_country("BEN"),
        "Bénin — douanes.gouv.bj",
        "6 129 pos.",
    ),
    "BFA": (lambda: ecowas_converter.convert_country("BFA"), "Burkina — dgi.bf", "6 129 pos."),
    "CIV": (
        lambda: ecowas_converter.convert_country("CIV"),
        "Côte d'Ivoire — guce.gouv.ci",
        "6 129 pos.",
    ),
    "GIN": (lambda: ecowas_converter.convert_country("GIN"), "Guinée — dgd.gov.gn", "6 129 pos."),
    "MLI": (
        lambda: ecowas_converter.convert_country("MLI"),
        "Mali — douanes.gouv.ml",
        "6 129 pos.",
    ),
    "NER": (
        lambda: ecowas_converter.convert_country("NER"),
        "Niger — impots.gouv.ne",
        "6 129 pos.",
    ),
    ...
    "CPV": (
        lambda: ecowas_converter.convert_country("CPV"),
        "Cabo Verde — dérivé TEC CEDEAO",
        "6 129 pos.",
    ),
    "GHA": (
        lambda: ecowas_converter.convert_country("GHA"),
        "Ghana — dérivé TEC CEDEAO",
        "6 129 pos.",
    ),
    "GMB": (
        lambda: ecowas_converter.convert_country("GMB"),
        "Gambie — dérivé TEC CEDEAO",
        "6 129 pos.",
    ),
    "GNB": (
        lambda: ecowas_converter.convert_country("GNB"),
        "Guinée-Bissau — dérivé TEC CEDEAO",
        "6 129 pos.",
    ),
    "LBR": (
        lambda: ecowas_converter.convert_country("LBR"),
        "Liberia — dérivé TEC CEDEAO",
        "6 129 pos.",
    ),
    "SLE": (
        lambda: ecowas_converter.convert_country("SLE"),
        "Sierra Leone — dérivé TEC CEDEAO",
        "6 129 pos.",
    ),
    # CEMAC
    ...
    "GNQ": (
        lambda: cemac_converter.convert_country("GNQ"),
        "Guinée Équatoriale — dérivé TEC CEMAC",
        "5 239 pos.",
    ),
    # Afrique de l'Ouest
