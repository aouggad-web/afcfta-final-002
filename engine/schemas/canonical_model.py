...
class Provenance(BaseModel):
    ...
    data_status: DataStatus = Field(
        DataStatus.SYNTHETIC, description="VERIFIED / PARTIAL / SYNTHETIC"
    )
    reliability: ReliabilityGrade = Field(
        ReliabilityGrade.D, description="Note de fiabilité A/B/C/D"
    )
    source_name: Optional[str] = Field(
        None, description="Nom de la source (ex: 'DGD Algérie — Tarif intégré')"
    )
    source_url: Optional[str] = Field(None, description="URL de la source officielle")
    source_document: Optional[str] = Field(
        None, description="Document précis (ex: 'LF 2026, art. 12' ou 'EAC CET 2022')"
    )
    version_date: Optional[date] = Field(
        None, description="Date de version du tarif source (millésime)"
    )
    retrieved_at: Optional[datetime] = Field(None, description="Date/heure de collecte")
    notes: Optional[str] = Field(
        None, description="Remarques (méthode de collecte, limites connues)"
    )
...
class CommodityCode(BaseModel):
    ...
    hs_version: Optional[str] = Field(
        None, description="v4 — Version SH de la nomenclature (ex: 'HS2022')"
    )
    description_official_fr: Optional[str] = Field(
        None, description="v4 — Libellé officiel exact du tarif national (non reformulé)"
    )
...
class Measure(BaseModel):
    ...
    rate_pct: Optional[float] = Field(
        None, description="Taux ad valorem en % (None si purement spécifique)"
    )
    ...
    rate_type: RateType = Field(
        RateType.AD_VALOREM, description="AD_VALOREM / SPECIFIC / MIXED / EXEMPT"
    )
    specific_amount: Optional[float] = Field(
        None, description="Montant du droit spécifique (si SPECIFIC ou MIXED)"
    )
    specific_unit: Optional[str] = Field(
        None, description="Unité du droit spécifique (ex: 'DZD/kg', 'EGP/unité')"
    )
    basis: DutyBasis = Field(DutyBasis.CIF, description="Assiette de calcul")
    basis_includes: List[str] = Field(
    ...
    basis_note: Optional[str] = Field(None, description="Précision sur l'assiette si basis=OTHER")
    sequence: int = Field(
        100, description="Ordre d'application (10=DD, 20-80=taxes intermédiaires, 90=TVA)"
    )
    legal_reference: Optional[str] = Field(
        None, description="Base légale (ex: 'Art. 16 Code des Douanes', 'Circ. 419 DGD')"
    )
...
class Requirement(BaseModel):
    ...
    issuing_authority: Optional[str] = Field(
        None, description="Autorité qui délivre le document (dénomination complète)"
    )

    # ---- v4 ----
    issuing_authority_code: Optional[str] = Field(
        None, description="Sigle de l'autorité (ex: 'ONSSA', 'NAFDAC', 'DGD')"
    )
    applies_to: str = Field("IMPORT", description="IMPORT / EXPORT / BOTH")
    legal_reference: Optional[str] = Field(None, description="Base légale de l'exigence")
    when_required: Optional[str] = Field(
        None,
...
class FiscalAdvantage(BaseModel):
    ...
    agreement: Optional[str] = Field(
        None, description="v4 — Accord concerné (ex: 'ZLECAf', 'ZALE', 'UE-Maroc')"
    )
    required_document: Optional[str] = Field(
        None, description="v4 — Document exigé (ex: 'Certificat d'origine ZLECAf')"
    )
...
class RegulatoryEngineResponse(BaseModel):
    ...
    data_status: Optional[DataStatus] = Field(
        None, description="v4 — Statut remonté en tête de réponse pour l'UI"
    )
    disclaimer: Optional[str] = Field(
        None, description="v4 — Mention légale obligatoire si SYNTHETIC/PARTIAL"
    )
