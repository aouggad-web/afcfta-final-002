...
def migrate_country(engine, iso3: str) -> int:
    ...
    with engine.connect() as conn:
        ...
        conn.execute(
            text("UPDATE countries SET total_positions = :c WHERE iso3 = :iso3"),
            {"c": count, "iso3": iso3},
        )
        conn.commit()
...
def main():
    ...
    with engine.connect() as conn:
        try:
            conn.execute(
                text(
                    "CREATE INDEX IF NOT EXISTS idx_desc_search ON commodities USING gin(to_tsvector('french', description_fr));"
                )
            )
            conn.commit()
