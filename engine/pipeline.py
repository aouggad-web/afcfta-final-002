        ...
        for iso3 in countries:
            if iso3 not in self.adapters:
                ...
                results["countries_failed"].append(
                    {"country": iso3, "error": "No adapter available"}
                )
                continue

            try:
                count = self._process_country(iso3)
                results["countries_processed"].append({"country": iso3, "records": count})
                results["total_records"] += count
                print(f"[OK] {iso3}: {count} enregistrements")
            except Exception as e:
                print(f"[ERROR] {iso3}: {str(e)}")
                results["countries_failed"].append({"country": iso3, "error": str(e)})
        ...
        with open(output_file, "w", encoding="utf-8") as f:
            for line_num, record in enumerate(adapter.transform()):
                ...
                if hs6 not in hs6_index:
                    hs6_index[hs6] = []
                hs6_index[hs6].append({"national_code": nat_code, "line": line_num})
