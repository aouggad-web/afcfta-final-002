...
def migrate_country(engine, iso3: str) -> int:
    ...
    with engine.connect() as conn:
        ...
        conn.execute(
            text("UPDATE countries SET total_positions = :count WHERE iso3 = :iso3"),
            {"count": count, "iso3": iso3},
        )
        conn.commit()
...
def main():
    ...
    with engine.connect() as conn:
        try:
            conn.execute(
                text(
                    "CREATE INDEX IF NOT EXISTS idx_desc_ft ON commodities USING gin(to_tsvector('french', description_fr));"
                )
            )
            conn.commit()
            print("✓ Index créé")
