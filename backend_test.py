...
def test_health():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            log_test("GET /api/health", False, error=f"HTTP {response.status_code}: {response.text[:200]}")
            return False
        ...
        if response.status_code == 200:
            ...
            if isinstance(data, list) and len(data) == 54:
                ...
                if "code" in first and "name" in first and "region" in first:
                    ...
                    log_test("GET /api/countries", False, error="Missing expected fields in country data")
                    return False
            else:
                log_test("GET /api/countries", False, error=f"Expected 54 countries, got {len(data) if isinstance(data, list) else 'non-list'}")
                return False
        else:
            log_test("GET /api/countries", False, error=f"HTTP {response.status_code}: {response.text[:200]}")
            return False
    except Exception as e:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "country_code" in data and "country_name" in data:
                    log_test(f"GET /api/country-profile/{code}", True, f"Country: {data.get('country_name')}")
                else:
                    log_test(f"GET /api/country-profile/{code}", False, error="Missing expected fields")
                    return False
            else:
                log_test(f"GET /api/country-profile/{code}", False, error=f"HTTP {response.status_code}: {response.text[:200]}")
                return False
        except Exception as e:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "normal_tariff_rate" in data and "hs_code" in data:
                log_test("POST /api/calculate-tariff", True, 
                        f"Tariff calculated: {data.get('normal_tariff_rate')*100:.2f}% for HS {data.get('hs_code')}")
                return True
            else:
                log_test("POST /api/calculate-tariff", False, error="Missing expected fields in response")
                return False
        else:
            log_test("POST /api/calculate-tariff", False, error=f"HTTP {response.status_code}: {response.text[:200]}")
            return False
    except Exception as e:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "hs_code" in data and "status" in data:
                    log_test(f"GET /api/rules-of-origin/{hs_code}", True, 
                            f"Status: {data.get('status')}, Chapter: {data.get('chapter')}")
                else:
                    log_test(f"GET /api/rules-of-origin/{hs_code}", False, error="Missing expected fields")
                    return False
            else:
                log_test(f"GET /api/rules-of-origin/{hs_code}", False, error=f"HTTP {response.status_code}: {response.text[:200]}")
                return False
        except Exception as e:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "overview" in data and "top_exporters_2024" in data:
                ...
                log_test("GET /api/statistics", True, 
            ...
            log_test("GET /api/statistics", False, error=f"HTTP {response.status_code}: {response.text[:200]}")
            return False
    except Exception as e:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "email" in data and data.get("email") == ADMIN_EMAIL.lower():
                log_test("POST /api/auth/login (admin)", True, 
                        f"Admin logged in: {data.get('name')} ({data.get('role')})")
                return True
            else:
                log_test("POST /api/auth/login (admin)", False, error="Unexpected response structure")
                return False
        else:
            log_test("POST /api/auth/login (admin)", False, error=f"HTTP {response.status_code}: {response.text[:200]}")
            return False
    except Exception as e:
...
def check_backend_logs():
    ...
    try:
        ...
        critical_errors = []
        error_keywords = ["ModuleNotFoundError", "ImportError", "500 Internal Server Error", "Traceback", "ERROR"]
